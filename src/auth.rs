...
impl AuthConfig {
    pub fn from_env(
        ...
        if let Some(secret) = secret {
            ...
            let allowed_roles = parse_role_set(&env_string(
                "EAL_JWT_ALLOWED_ROLES",
...
impl JwtVerifier {
    fn verify(&self, token: &str, now: i64) -> Result<AuthContext, ()> {
        ...
        let protected = segments.next().filter(|value| !value.is_empty()).ok_or(())?;
        let claims = segments.next().filter(|value| !value.is_empty()).ok_or(())?;
        let signature = segments.next().filter(|value| !value.is_empty()).ok_or(())?;
...
mod tests {
    ...
    fn rejects_expired_wrong_audience_and_modified_tokens() {
        ...
        assert!(config()
            .authenticate_at(&headers, 1_950_000_000)
            .is_err());
    ...
    fn rejects_disallowed_roles_and_conflicting_credentials() {
        let tenant_id = Uuid::new_v4();
        let token = token(claims(tenant_id, "service_role"));
        let mut headers = HeaderMap::new();
        headers.insert(
            AUTHORIZATION,
            HeaderValue::from_str(&format!("Bearer {token}")).unwrap(),
    ...
    fn insecure_mode_requires_explicit_subject_and_roles() {
        ...
        assert!(AuthConfig::insecure_headers().authenticate(&headers).is_err());
