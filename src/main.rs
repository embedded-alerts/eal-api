        ...
        use eal_interfaces::{CreateAlertRule, AlertRule, AlertRuleEvent};
        use serde::Serialize;
        ...
        #[derive(Clone)]
        struct AppState {
            records: Arc<RwLock<BTreeMap<Uuid, AlertRule>>>,
        ...
        async fn main() -> Result<(), Box<dyn std::error::Error>> {
            ...
            let app = Router::new()
                ...
                .route("/api/v1/alert-rules", get(list_records).post(create_record))
                .route("/api/v1/alert-rules/{id}", get(get_record))
            ...
            info!(%addr, "Embedded Alerts API listening");
        ...
        async fn index() -> Json<serde_json::Value> {
            Json(serde_json::json!({
                "service": "eal-api",
                "product": "Embedded Alerts",
                "rest": "/api/v1/alert-rules",
                "websocket": "/ws"
            }))
        }

        async fn health() -> Json<Health<'static>> {
            Json(Health { status: "ok", service: "eal-api", version: env!("CARGO_PKG_VERSION") })
        }

        async fn metrics(State(state): State<AppState>) -> String {
            format!("# TYPE eal_records gauge
eal_records {}
", state.records.read().await.len())
        ...
        async fn get_record(Path(id): Path<Uuid>, State(state): State<AppState>) -> Result<Json<AlertRule>, ApiError> {
        ...
        async fn create_record(
            ...
            let event = AlertRuleEvent {
                event_id: Uuid::new_v4(),
                event_type: "alert_rule.created".into(),
                occurred_at: now,
                data: record.clone(),
            };
